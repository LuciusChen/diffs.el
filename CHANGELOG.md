...
- Keep note-box borders aligned for mixed ASCII, CJK, Emoji, and image placeholders by deriving the right edge entirely from current-buffer pixel metrics instead of mixing frame columns with pixels, then reproject them after text scaling or theme changes alter display metrics.
- Let old/new comments anchored to the same aligned split row share one vertical lane, padding only the shorter side instead of stacking each comment with a full-height empty peer block.
- Make automatic stacked laziness and split paging account for scan-time changed-content work as well as raw line/row counts. On the 16-file complex benchmark this moves expensive alignment and refinement off the command path, reducing stacked setup from about 1.47 seconds to 15 ms and first split from about 1.02 seconds to 72 ms while preserving explicit eager/complete overrides.
- Run both volume and complex-content profiles from the default benchmark, with the latter covering multi-hunk unequal and reordered replacements, token-rich Unicode lines, long-line guards, and isolated cold hunk pairing/refinement timings.
