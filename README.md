...
| `diffs-files` | Command | Any two saved files, with the first as file A on the left and the second as file B on the right |
...
`M-x diffs-files` first prompts for file A on the left and then file B on the right. The files may have different names and live in different directories or outside version control. The command uses Emacs's configured `diff-command` to compare their saved contents, then opens the same stacked or split review used by every other diffs.el entry. A stacked header shows `A · first → B · second`; each split column shows only its own A or B path, including in paged and sticky headers. Long header paths elide middle directories while retaining their identifying suffixes; source coordinates always keep the complete paths.

Source navigation remains side-specific: `RET` or `M-RET` on A visits the first file, while the same commands on B visit the second. Displayed comments use their side's path and an A/B line anchor. Internally both sides stay in one comparison section: A comments carry old ranges, while file B is the canonical file path exposed to review agents, preserving the normal `oldRange`/`newRange` API. `e` loads unchanged context from both files, `g` rereads the same paths, and `q` restores the entry layout and position. Review decisions are previews until explicitly applied with `C-c C-c`; confirmation names file B because application targets only that file and never saves or stages it.
...
The default view splits the frame into two synchronized windows — old on the left, new on the right — with row-perfect alignment (dimmed filler bands pad insertions and deletions), per-side line numbers, and the same native refinement and syntax faces as the unified view. In both stacked and split layouts, theme-native added/removed backgrounds run continuously from the fringe bar through the line-number gutter and extend to the right edge. Changed line-number digits inherit the standard added/removed indicator colors while context numbers remain subdued; digit color remains even when split full-width backgrounds are disabled. `n`/`p` move by hunk in both windows, `RET` visits the source, `s` returns to the unified view, and `q` closes both columns and quits the complete diff view.
...
File headers, line gutters, and hunk separators have one public presentation function each. Every function receives a copied context plist and returns a string or nil; returning nil suppresses that element. The same functions receive `:view` as `stacked` or `split`, so one configuration covers both layouts. File-header contexts additionally expose `:independent-sources`; `:side` is `old` or `new` for the two split columns of an independent A/B file comparison and nil otherwise. Hunk separator contexts include `:direction` as `up` or `down` together with `:count`, `:visible`, `:hidden`, and `:context-step`. Gutter functions run on materialized rows and should remain pure and fast.
